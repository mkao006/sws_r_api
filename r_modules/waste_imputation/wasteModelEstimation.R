...
## Tests
## ---------------------------------------------------------------------
## test = getOfficialLossData()
## test2 = test[, length(Value_measuredElement_5120),
##     by = c("measuredItemCPC", "timePointYears")]

## library(lattice)
## xyplot(V1 ~ as.numeric(timePointYears)|measuredItemCPC, data = test2, type = "h")


## test3 = test2[timePointYears >= 2000, ]
## xyplot(V1 ~ as.numeric(timePointYears)|measuredItemCPC, data = test3, type = "h")
